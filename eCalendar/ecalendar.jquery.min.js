...
$.fn.ECalendar = function(A) {
    ...
    C = A.callback;
    if (!I()) { if (A.type == 0) { b.attr("type", "datetime-local") } else { b.attr("type", "date") } if (callbackk) { b.change(function() { C(b.val(), z) }) } return false } A.format = A.type == 0 ? w : F;
    if (A.skin === false) { A.skin = Math.floor(Math.random() * G.length) } A.skin = G[A.skin] ? G[A.skin] : A.skin;
    J();
    b.click(function(O) {
        var y = b.attr("data-ec") ? b.attr("data-ec") : b.val();
        var M = y ? new Date(y.replace(/-/g, "/")) : new Date();
        if (M == "Invalid Date") { M = new Date() } p(M);
        var P = b.offset().top - $(document).scrollTop();
        var x = $(window).height();
        var Q = b.offset();
        var N = b.position();
        if (x / 2 < P) { z.css({ "left": N.left + A.offset[0], "top": N.top - A.offset[1] - z.height() - 1, }) } else { z.css({ "left": N.left + A.offset[0], "top": N.top + b.outerHeight() + A.offset[1], }) } b.parent(".calendarWarp").css("zIndex", +10);
        z.show()
    });
    z.mousedown(function(x) { x.stopPropagation() });
    $(document).mousedown(function() { if (z.is(":visible")) { z.hide(0, function() { if (C) { C(a.val(), z) } }).parent(".calendarWarp").css("zIndex", 1) } });
    ...
    o.click(function() { var x = $(this); if (x.index() == 0) { L -= 2 } p(new Date(k, L, m, K, h)); if (C) { C(a.val(), z) } });
    $(j).mousedown(function(M) {
        ...
        c.prepend(x);
        c.append(y);
        c.children("i").each(function() { var N = parseInt($(this).attr("data")); if (N > 12) { $(this).text(12).hide() } if (N < 1) { $(this).text(1).hide() } });
            ...
            if (c[0].tagName == "LI") {
                if (Math.abs(e) > 1 || Math.abs(g) > 1) {
                    ...
                    if (Math.abs(e) > 5 || Math.abs(g) > 5) {
                        ...
                        N += 12
                    } else { c.children("div").attr("class", "at12") } K = N;
                    h = y;
                ...
                var R = e;
                var M = E + R;
                if (c.hasClass("imonth") && M >= 1 && M <= 12) { c.children("i").each(function() { var U = parseInt($(this).attr("data")) + R; if (U > 12 || U < 1) { $(this).hide() } else { $(this).text(U).show() } }); if (M == 12) { c.children("span").nextAll().hide() } else { if (M == 1) { c.children("span").prevAll().hide() } } c.children("span").text(M) }
                if (c.hasClass("iyear")) {
    ...
    $(document).mouseup(function() {
        if (c) {
            if (c[0].tagName == "LI") {
                ...
                z.hide(0, function() { if (C) { C(a.val(), z) } }).parent(".calendarWarp").css("zIndex", 1)
            } else { c.children("i").remove(); var x = c.text(); if (c.hasClass("imonth")) { p(new Date(k, x - 1, m, K, h)) } if (c.hasClass("iyear")) { p(new Date(x, L - 1, m, K, h)) } d.animate({ opacity: 1 }, 600); if (C) { C(a.val(), z) } } c = false;
    ...
    function p(M) {
        ...
        if (z.data("date") != (k + "-" + L)) {
            ...
            var Q = "";
            for (var N = 1; N <= R; N++) { if (N < T) { Q += "<li class='other prve'>" + (O - (T - N) + 1) + "</li>" } else { if (N >= T && N < P + T) { if ((N - T + 1) == m) { Q += "<li class='activ' data='" + (N - T + 1) + "'>" + (N - T + 1) + "</li>" } else { Q += "<li data='" + (N - T + 1) + "'>" + (N - T + 1) + "</li>" } } else { Q += "<li class='other next'>" + (N - (P + T) + 1) + "</li>" } } } q.html(Q)
        }
        z.find(".activ").removeClass("activ").removeAttr("style");
        if (cur == m) { q.children("li[data=" + m + "]").css("overflow", "visible"); }
        cur = m
        ...
        var U = "";
        if (S == 0) { U = "今天" } else { U = "<span class='itime'>" + Math.abs(S) + "</span> "; if (S > 0) { U += "天后" } else { U += "天前" } } v.html(U);
    ...
    function I() { var x = navigator.userAgent; var N = ["Android", "iPhone", "SymbianOS", "Windows Phone", "iPad", "iPod"]; var y = true; for (var M = 0; M < N.length; M++) { if (x.indexOf(N[M]) > 0) { y = false; break } } return y }
};